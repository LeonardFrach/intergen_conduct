...
load("data/combinedData_new2.RData") # non-imputed data
```
...
# Correlation between age 8 and age 14 items

scales <- MIdata %>% 
  select(c("NN111":"NN118", "UB127":"UB134")) %>%
  mutate_all(., as.numeric)

cor_scales <- cor(scales, use = "pairwise.complete.obs", method = "spearman")
cor_scales <- cor_scales[9:16, 1:8]


data_cor <- MIdata %>% 
  mutate_at(c("NN111","NN112", "NN113", "NN114", "NN115", "NN116", "NN117","NN118", 
              "UB127","UB128", "UB129", "UB130", "UB131", "UB132", "UB133","UB134"), as.numeric) 


# correlation between latent factors

mod_cor = c(
  "conduct_8 =~ NN111 + NN112 + NN113 + NN114 + NN115 + NN116 + NN117 + NN118",
  "conduct_14 =~ UB127 + UB128 + UB129 + UB130 + UB131 + UB132 + UB133 + UB134",
  
  "conduct_8 ~~ conduct_14"
)


cor_factors <- cfa(model = mod_cor, data = data_cor, missing = "fiml")
# only using individuals with some phenotypic data

pe_cor <- parameterEstimates(cor_factors, standardized = T)
# correlation between latent factors = 0.23
# without FIML correlation is almost the same (0.22)
...
genetic_transmis_14y$Class <- rep(child_effects_14y$Class, times = 2)
...
png("Figure_main_complete_14y_8k.png", height = 5700, width = 7700, res = 900, type = "cairo")
plot3b + 
  scale_y_discrete(limits = rev) + 
  scale_colour_manual(values = pal) + 
...
pdf("Figure_main_complete_14y.pdf")
plot3b + 
  scale_y_discrete(limits = rev) + 
  scale_colour_manual(values = pal) + 
...
plot3a2 <- plot3 + 
  scale_y_discrete(limits = rev) + 
  scale_colour_manual(values = pal) + 
  scale_x_continuous(limits = c(-.08, .11), oob = scales::squish) +
  theme(text = element_text(size = 15),
